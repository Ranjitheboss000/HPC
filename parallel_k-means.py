import math
import csv
import time
import numpy as np
import collections
from mpi4py import MPI
from sklearn.cluster import KMeans
from sklearn.metrics.cluster import adjusted_rand_score

#===================================================Divide data set to further scattering=====================
def chunkIt(seq, num):
    avg = len(seq) / float(num)
    out = []
    last = 0.0

    while last < len(seq):
        out.append(seq[int(last):int(last + avg)])
        last += avg
    return out

#===================================================Count lables for recalculating means of centroids=====================	
def addCounter(counter1, counter2):
    for item in counter2:
        counter1[item] += counter2[item]
    return counter1

comm = MPI.COMM_WORLD
size = comm.Get_size()
rank = comm.Get_rank()

global dimensions, num_clusters, num_points,data,flag
num_clusters=0

if rank==0: 		#master part
	#===============================================reading and preparing data set======================
	
	print("Enter the number of clusters you want to make: ")
	num_clusters = int(input())
	with open('3D_spatial_network.csv','r') as f:
		reader = csv.reader(f)
		data = list(reader)
		
	data.pop(0)
	for i in range (len(data)):
		data[i].pop(0)

	data=data[:10000]
	data=np.array(data).astype(np.float)

	kmeans = KMeans(n_clusters=num_clusters, random_state=0).fit(data).labels_
	#====================================================================================================

	start_time = time.time() 	#turn on a timer which allows us to estimate performane of this algorithm

	#================================================Initialize centroids matrix=========================
	initial = np.vstack([data[i] for i in range(num_clusters)])
	#====================================================================================================

	num_points = len(data)                                    #number of rows
	dimensions = len(data[0])                                 #number of columns
	
	chunks=chunkIt(data,size)								  #dividing data set on parts for further scattering
	
	#====================================================================================================
else:														  #workers part
	chunks = None											  #initialize variable
	initial = None											  #initialize variable
	data = None											      #initialize variable
	dimensions = None										  #initialize variable
	num_points = None										  #initialize variable
	cluster_vector= None									  #initialize variable
	Q_clust= None											  #initialize variable
	num_clusters= None										  #initialize variable
	centroid=None
	kmeans= None
	start_time=None
	#====================================================================================================

data=comm.scatter(chunks, root=0)							  #send chunks of data set to the workers
num_clusters=comm.bcast(num_clusters,root=0)
dimensions=comm.bcast(dimensions,root=0)
initial=comm.bcast(initial, root = 0)						  #send centroid matrix to the workers
flag= True

while flag:
	clusters=[]											  		  #initialize variable
	cluster_vector=[]

	#===================================Calculating dist matrix in each process==============================
	dist = np.zeros((len(data),num_clusters))


	for j in range(num_clusters):
		for i in range(len(data)):
			dist[i][j]=np.linalg.norm(initial[j]-data[i])

	#=======================initialize lable for each sample in each process======================
	for i in range(len(dist)):										#iterable take each raw in dist matrix and 
		clusters.append(np.argmin(dist[i])+1)                       #find column index of min value (this index is number of centroid)

	#===================================Calculating the number of samples in each cluster_vector====================
	Q_clusts=collections.Counter(clusters)							
	
	#===================================Summing the number of samples for each cluster_vector=======================
	gathered_Q = comm.allgather(Q_clusts)
	totcounter = gathered_Q[0]
	for i in gathered_Q[1:]: totcounter = addCounter(totcounter,i)

	comm.Barrier()

	centroids=np.zeros((num_clusters,dimensions))
	for k in range (1,num_clusters+1):
		indices = [i for i, j in enumerate(clusters) if j == k] # indices of those data points which belong to k th cluster 
		centroids[k-1]=np.divide((np.sum([data[i] for i in indices], axis=0)).astype(np.float),totcounter[k])

	centroid = comm.allreduce(centroids,MPI.SUM)
	comm.Barrier()	
	
	#====================================From each worker we gather cluster_vector vector and join them ============
	cluster_vector=comm.gather(clusters, root=0)
	comm.Barrier()

	#==========================================================================================================
	if rank==0:
		cluster_vector=[item for sublist in cluster_vector for item in sublist]

	if np.all(centroid==initial):
		flag=False
		if rank == 0: 
			print ("Execution time %s seconds" % (time.time() - start_time))
			print('adjusted_rand_score',adjusted_rand_score(kmeans,cluster_vector))
		
	else: initial= centroid 
	comm.Barrier()




			

		
